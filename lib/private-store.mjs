...
export function createPrivateStore(sql, options = {}) {
  ...
  async function ensureDefaultWeeklyTasks() {
    const rows = await sql`SELECT COUNT(*)::int AS count FROM weekly_checklist_tasks`;
    if (Number(rows[0]?.count || 0) > 0) return;

    for (const [index, task] of DEFAULT_WEEKLY_TASKS.entries()) {
      await sql`
        INSERT INTO weekly_checklist_tasks (id, label, position, active)
        VALUES (${String(task.id)}, ${task.label}, ${index}, TRUE)
        ON CONFLICT (id) DO NOTHING
      `;
    }
  ...
  async function maybeAwardWeeklyIncomeMultiplier(dateKey) {
    ...
    const weeklyTasks = await listWeeklyChecklistTasks(weekStart);
    if (!isWeeklyMultiplierEligible(completionRows.map(row => row.award_date), weeklyTasks, weekStart)) {
      const removedRows = await sql`
        DELETE FROM checklist_weekly_multipliers
        WHERE week_start = ${weekStart}
        RETURNING money_entry_id
      `;
      for (const row of removedRows) {
        if (!row.money_entry_id) continue;
        await sql`
          DELETE FROM money_entries
          WHERE id = ${row.money_entry_id}
  ...
  async function listWeeklyChecklistTasks(weekStart) {
    await ensureDefaultWeeklyTasks();
    const rows = await sql`
      SELECT
        task.id,
        task.label,
        COALESCE(done.done, FALSE) AS done
      FROM weekly_checklist_tasks task
      LEFT JOIN weekly_checklist_completions done
        ON done.task_id = task.id
       AND done.week_start = ${weekStart}
      WHERE task.active = TRUE
      ORDER BY task.position ASC, task.created_at ASC
    `;
  ...
  async function getChecklistState() {
    ...
    return {
      ...
      tasks,
      weeklyTasks,
      weeklyComplete: isChecklistComplete(weeklyTasks),
  ...
  return {
    ...
    async setWeeklyChecklistTaskDone(id, done) {
      const dateKey = todayKey();
      const { start: weekStart } = weekRangeForDateKey(dateKey);
      await ensureDefaultWeeklyTasks();
      await sql`
        INSERT INTO weekly_checklist_completions (task_id, week_start, done, updated_at)
        VALUES (${String(id)}, ${weekStart}, ${Boolean(done)}, NOW())
    ...
    async addWeeklyChecklistTask(label) {
      const trimmedLabel = typeof label === "string" ? label.trim() : "";
      if (!trimmedLabel) {
        throw new Error("Task label is required.");
      }
    ...
    async removeWeeklyChecklistTask(id) {
      const dateKey = todayKey();
      await sql`
        UPDATE weekly_checklist_tasks
        SET active = FALSE
        WHERE id = ${String(id)}
      `;
